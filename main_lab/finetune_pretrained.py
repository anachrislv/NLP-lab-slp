...
####################### NOTE ########################
# Below we show the code using a loop.
# Due to GPU memory (and colab usage) constraints, we ran individually for each model using small batch size in colab

DATASETS = ['MR', 'Semeval2017A']  # 'MR' or 'Semeval2017A'
...
if __name__ == '__main__':
  ...
  for DATASET in DATASETS:
      if DATASET == "Semeval2017A":
          ...
          PRETRAINED_MODELS = PRETRAINED_MODEL_SEM
      elif DATASET == "MR":
          X_train, y_train, X_test, y_test = load_MR()
          PRETRAINED_MODELS = PRETRAINED_MODEL_MR
      else:
      ...
      for PRETRAINED_MODEL in PRETRAINED_MODELS:
        ...
        args = TrainingArguments(
          ...
          num_train_epochs=10, 
          per_device_train_batch_size=4, # small because cuda runs out of memory
          per_device_eval_batch_size=4,
      )
        trainer = Trainer(
            model=model,
            args=args,
            train_dataset=small_train_dataset, # train_dataset=tokenized_train_set
            eval_dataset=small_eval_dataset,   # eval_dataset=tokenized_test_set
