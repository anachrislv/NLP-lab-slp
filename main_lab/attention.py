...
class MultiHeadAttentionModel(nn.Module):
    ...
    def __init__(self, output_size, embeddings, max_length=60, n_head=3):
        ...
        self.mha = MultiHeadAttention(n_head, head_size, dim)
        self.ffwd = FeedFoward(dim)
        self.ln1 = nn.LayerNorm(dim)
        self.ln2 = nn.LayerNorm(dim)
    ...
    def forward(self, x, _):
        ...
        pos_emb = self.position_embedding_table(torch.arange(T).to(device))  # (T,C)
        x = tok_emb + pos_emb  # (B,T,C)
        x = x + self.mha(x)  # Apply multi-head attention
        x = x + self.ffwd(x)  # Apply feed-forward layer

        x = torch.mean(x, dim=1)  # Average pooling to get a sentence embedding
        logits = self.output(x)  # (C,output)
        return logits
